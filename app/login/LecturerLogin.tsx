...
import { lecturerLogin } from "@/network/lib/auth";
...
const LecturerLogin: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status === 200) {
        const lecturerData: LecturerData = { ...response.data.data, role: 'lecturer' }; 

        console.log('Lecturer data:', lecturerData);
        
  
        localStorage.setItem('token', response.data.token);
        localStorage.setItem('user', JSON.stringify(lecturerData));
  
        Swal.fire({
          title: 'Success!',
          text: 'You have successfully logged in as a lecturer!',
          icon: 'success',
          confirmButtonText: 'OK',
        }).then(() => {
          router.push('/'); // Redirect to lecturer home page
        });
  ...
  return (
    <div className="flex items-center justify-center">
      <div>
        <h2 className="text-2xl font-semibold text-center mb-6">Lecturer Login</h2>
